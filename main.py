...
def getwordladder(start: str, end: str) -> list[str] | None:
    ...
    if len(start) == 4:
        print("4 letters")
        word_graph = word_graph4
    elif len(start) == 5:
        print("5 letters")
        word_graph = word_graph5
    else:
...
async def exclude_word(word: Annotated[str, Path(min_length=4, max_length=5, title="Excluded word")]):
    if word in excluded_words:
        ...
        word_graph4 = sw.build_word_graph(words4 - excluded_words)
        word_graph5 = sw.build_word_graph(words5 - excluded_words)
